...
class ControllerPaymentNextpay extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('payment/nextpay', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->link('payment/nextpay', 'token=' . $this->session->data['token'], true);

		$data['cancel'] = $this->url->link('extension/payment', 'token=' . $this->session->data['token'], true);

		if (isset($this->request->post['nextpay_pin'])) {
			$data['nextpay_pin'] = $this->request->post['nextpay_pin'];
		} else {
			$data['nextpay_pin'] = $this->config->get('nextpay_pin');
		}
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'payment/nextpay')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}
		
		if (!$this->request->post['nextpay_pin']) {
